...
class TeacherDB:
    ...
    def update_teacher(self, user_id, name=None, email=None, specialization=None, hire_date=None, photo=None,
                       password=None):
        if not self.db.is_connected():
            return False
        cursor = None
        try:
            ...
            if specialization is not None:
                teacher_updates.append("specialization = %s")
                teacher_params.append(specialization)
            if hire_date is not None:
                teacher_updates.append("hire_date = %s")
                teacher_params.append(hire_date)
            if photo is not None:
                teacher_updates.append("photo = %s")
                teacher_params.append(photo)

            if teacher_updates:
                teacher_query = f"UPDATE teachers SET {', '.join(teacher_updates)} WHERE user_id = %s"
                teacher_params.append(user_id)
                cursor.execute(teacher_query, tuple(teacher_params))
    ...
    def remove_teacher(self, teacher_id):
        ...
        try:
            ...
            if cursor:
                cursor.close()
    def get_teacher_photo(self, teacher_id):
        if not self.db.is_connected():
    ...
    def record_attendance(self, seance_id, person_id, status, person_type='student'):
        ...
        try:
            ...
            if person_type == 'student':
                ...
                """
                print(f"Executing: {query} with ({seance_id}, {person_id}, {status})")
            else:  # teacher
                query = """
            ...
            logging.error(f"Error recording attendance for {person_type} {person_id}: {e}")
            self.db.connection.rollback()
            return False
        finally:
            if cursor:
